...
def get_playlist_collaboration(user, playlist):
    if not user or not user.is_authenticated:
        return None
    if user.is_staff or playlist.user_id == user.id:
        return None
...
def can_view_playlist(user, playlist):
    return bool(
        playlist.is_public
        or (user and user.is_authenticated and (user.is_staff or playlist.user_id == user.id))
        or get_playlist_collaboration(user, playlist)
    )
...
def can_manage_playlist(user, playlist):
    return bool(user and user.is_authenticated and (user.is_staff or playlist.user_id == user.id))


class SongWritePermission(BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        return bool(
            ...
            and (request.user.is_staff or obj.uploaded_by_id == request.user.id)
        )
